...
function wopofc_shortcode( $atts = [], $content = null) {
    ...
    if (isset($atts['output'])){
        $outputExt = sanitize_text_field(str_replace('.','',$atts['output'])); // remove dot in out put file extension
    }
    ...
    if (isset($_FILES['file'])){
      ...
      if (empty($options['api_key'])){
        _e('<p style="color:red">Error: API key is empty.</p>','wopo-file-converter');
      }else if ($call_api){
    ...
    <form action="<?php the_permalink() ?>" method="POST" enctype="multipart/form-data" class="wopo-file-converter">        
        <input type="file" name="file" class="file" <?php do_action('wopofc_form_input_file_attr',$atts) ?> >
        <?php if (!(isset($atts['output']) && in_array('wopo-file-converter-pro/wopo-file-converter-pro.php', get_option('active_plugins')))){ ?>
